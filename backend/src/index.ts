...
import { detectPII, redactPII, redactSensitiveData } from './utils/pii-detection';
import {
	getWhitelist,
	addToWhitelist,
	removeFromWhitelist,
...
const app = new Hono<{ Bindings: ENV; Variables: { userId: string; user: any } }>();
...
app.get('/v1/logs', async (c) => {
	try {
		...
		const logs = await getAuditLogsByDateRange(c.env, startDate, endDate, eventType, limit);
		return c.json({ logs });
	} catch (error) {
...
app.get('/v1/stats', async (c) => {
	try {
		...
		const totalAiScans = detections.filter((d) => d.type.startsWith('AI_SCAN_')).length;
...
app.post('/v1/analyze-context', async (c) => {
	...
	try {
		...
		const validFieldTypes = ['name', 'email', 'address', 'phone', 'freeform', 'unknown'];
		if (body.fieldType && !validFieldTypes.includes(body.fieldType)) {
			return c.json(
				{ error: 'Invalid fieldType. Must be one of: name, email, address, phone, freeform, unknown' },
				400,
			);
		}
		...
		const contextInfo = body.context ? `Context: This text is being entered into a "${body.context}" field.` : '';
		const fieldTypeInfo =
			body.fieldType && body.fieldType !== 'unknown' && body.fieldType !== 'freeform'
...
"${body.text}"`,
		...
		if (body.fieldType && analysis.detections && Array.isArray(analysis.detections)) {
			...
			if (expected.length > 0 && analysis.detections.length > 0) {
				const unexpectedDetections = analysis.detections.filter((d: any) => !expected.includes(d.type));
		...
		for (const patternDet of patternDetections) {
			const isDuplicate = analysis.detections.some(
				(d: any) => d.type === patternDet.type && d.value.toLowerCase() === patternDet.value.toLowerCase(),
			);
			if (!isDuplicate) {
		...
		if (analysis.detections.length > 0) {
			...
			analysis.confidence = Math.max(...analysis.detections.map((d: any) => d.confidence));
		}
